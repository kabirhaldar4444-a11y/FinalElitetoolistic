...
const CreateUser = ({ user, profile, initialRole = 'candidate', isPage = false }) => {
  ...
  const handleCreateCandidate = async (e) => {
    ...
    try {
      ...
      const { data, error: createError } = await supabaseAdmin.auth.signUp({
        ...
        options: {
          // Skip sending confirmation email — admin is creating this account directly
      ...
      // ✅ CRITICAL: Insert profile row so user can login
      // The admin's main supabase client (with session) has permission to insert
      if (data?.user) {
        ...
        if (profileError) {
          ...
          // Don't block success — profile can be fixed via SQL
        }
      }
      ...
      <div className="min-h-screen relative overflow-x-hidden font-sans">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 py-12 md:py-16">
          ...
          <div className="mb-12 animate-fade-in">
            ...
            <p className="mt-2 font-medium text-[color:var(--text-light)]">
              {role === 'admin' ? 'Register a new administrative staff member.' : 'Create a new candidate login account.'}
            </p>
          </div>

          <div className="glass-card-saas w-full max-w-2xl mx-auto p-8 md:p-12 relative overflow-hidden group">
